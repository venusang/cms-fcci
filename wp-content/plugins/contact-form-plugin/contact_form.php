...
if( ! function_exists( 'cntctfrm_display_form' ) ) {
	function cntctfrm_display_form( $atts = array( 'lang' => 'en' ) ) {
		...
		// $page_url ='http://localhost:8888/fcci/contact/contact-us/'; //for local testing
		...
		if( true === $cntctfrm_result ) {
			...
			if( $cntctfrm_options['cntctfrm_action_after_send'] == 1 )
				$content .= '<div id="contactusThankYouMsg">'.$cntctfrm_options['cntctfrm_thank_text'][$lang].'</div>';
			else
		...
		if( true !== $cntctfrm_result) { 
			...
			// Output form
			$content .='<span id="contactusRequiredFields">* Indicates required fields</span>';
			...
			$content .= '<div style="text-align: left;">
					<textarea rows="4" cols="30" name="cntctfrm_contact_message" id="cntctfrm_contact_message">'.$message.'</textarea>
				</div>';
